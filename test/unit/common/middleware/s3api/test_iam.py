...
class TestS3Iam(TestCase):
    ...
    def test_allow_all_from_whitelisted_ip(self):
        rules = json.loads("""
        {
            "Statement": [{
                "Sid": "AllowAllFromWhitelistedIp",
                "Effect": "Allow",
                "Principal": "*",
                "Action": "s3:*",
                "Resource": ["*"],
                "Condition": {
                    "IpAddress": {
                        "aws:SourceIp": "127.0.0.1/24"
                    }
                }
            }]
        }
        """)
        rsc = IamResource("my-company")
        check = IamRulesMatcher(rules)
        self.assertEqual(
            ('ALLOW', 'AllowAllFromWhitelistedIp'),
            check(
                rsc,
                "s3:ListBucket",
                MagicMock(
                    headers={'x-cluster-client-ip': '127.0.0.1'})
            )
        )
        ...
        self.assertEqual(
            ...
            check(
                ...
                "s3:ListBucket",
                MagicMock(
                    headers={'x-cluster-client-ip': '127.1.0.1'})
            )
        )

    def test_allow_all_except_black_listed_ip(self):
        rules = json.loads("""
        {
            "Statement": [{
                "Sid": "AllowAllExceptBlacklistedIp",
                "Effect": "Allow",
                "Principal": "*",
                "Action": "s3:*",
                "Resource": ["*"],
        ...
        self.assertEqual(
            ('ALLOW', 'AllowAllExceptBlacklistedIp'),
            check(
                rsc,
                "s3:ListBucket",
                MagicMock(
    ...
    def test_deny_all_non_white_listed_ip(self):
        rules = json.loads("""
        {
            "Statement": [{
                "Sid": "DenyAllNonWhitelistedIPs",
                "Effect": "Deny",
                "Principal": "*",
                "Action": "s3:*",
                "Resource": [
        ...
        """)
        rsc = IamResource("example-bucket")
        check = IamRulesMatcher(rules)
        self.assertEqual(
            (None, None),
            check(
        ...
        self.assertEqual(
            ...
            check(
                ...
                "s3:ListBucket",
                MagicMock(
                    headers={'x-cluster-client-ip': '198.51.100.10'})
            )
        )

        self.assertEqual(
            ("DENY", "DenyAllNonWhitelistedIPs"),
            check(
                rsc,
                "s3:ListBucket",
                MagicMock(
    ...
    def test_deny_all_from_black_listed_ip(self):
        rules = json.loads("""
        {
            "Statement": [{
                "Sid": "DenyAllFromBlacklistedIp",
                "Effect": "Deny",
                "Principal": "*",
                "Action": "s3:*",
                "Resource": ["*"],
