...
servers_per_port_type            port        When servers_per_port is enabled, you can
                                             choose on the ports the object-server will
                                             bind. It can be the standard port, by
                                             setting 'port', or the replication_port,
                                             by setting 'replication_port', or both,
                                             by setting both values separated by a
                                             comma. This is useful when you want to
                                             separate workers for client requests and
                                             replication requests, as replication
                                             requests can be IO-intensive and sometimes
                                             block the worker for few seconds (or more).
